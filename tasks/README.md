...
---- | ----------- | --------
`task001_quoref_question_generation`	| Writing questions that require tracking entity references	| Generation
`task002_quoref_answer_generation`	| Answering questions that require tracking entity references	| Generation
...
`task006_mctaco_question_generation_transient_stationary`	| Writing questions that involve commonsense understanding of "transient v. stationary" events.	Question | Generation
`task007_mctaco_answer_generation_transient_stationary`	| Answering questions that involve commonsense understanding of "transient v. stationary" events.	Answer | Generation
...
`task022_cosmosqa_passage_inappropriate_binary`	| Identifying inappropriate content in context | sentences.	
`task023_cosmosqa_question_generation`	| Craft one question such that it requires commonsense to be | answered.	
`task024_cosmosqa_answer_generation`	| Answering commonsense questions	| Generation
`task025_cosmosqa_incorrect_answer_generation`	| Writing incorrect answers options for a commonsense question.	Answer | Generation
`task026_drop_question_generation`	| Creating complex reasoning questions based on a passage.	| Generation
`task027_drop_answer_type_generation`	| Finding the answer type of a reasoning question	| Classification
`task028_drop_answer_generation`	| Answering a complex reasoning question based on a passage	| Generation
`task029_winogrande_full_object`	| Creating a pair of fill in the blank question-answer pairs on objects	Text | Modification
`task030_winogrande_full_person`	| Creating a pair of fill in the blank questions on persons	Text | Modification
`task031_winogrande_question_generation_object`	| Writing a fill in the blank question on objects	| Generation
`task032_winogrande_question_generation_person`	| Writing a fill in the blank question on persons	| Generation
`task033_winogrande_answer_generation`	| Answering a fill in the blank question on objects	| Generation
...
`task040_qasc_question_generation`	| Creating a question based on a given sentence.	| Generation
`task041_qasc_answer_generation`	| Writing correct answer to a given question based on a given sentence.	| Generation
`task042_qasc_incorrect_option_generation`	| Writing incorrect answers to a given question based on a given sentence	Answer | Generation
...
`task047_miscellaenous_answering_science_questions`	| Answering simple science questions	| Generation
`task048_multirc_question_generation`	| Constructing questions based on the information present in the passage	| Generation
...
`task051_multirc_correct_answer_single_sentence`	| Generating correct answer to single-sentence questions	| Generation
...
`task054_multirc_write_correct_answer`	| Writing A Correct Answer for a Reading Comprehension Task	| Generation
`task055_multirc_write_incorrect_answer`	| Writing Incorrect Answers for a Reading Comprehension Task	Answer | Generation
...
`task058_multirc_question_answering`	| Reading Comprehension Over Multiple Sentences	| Generation
`task059_ropes_story_generation`	| Generating a story about relations in the given paragraph	Text | Modification
`task060_ropes_question_generation`	| Constructing questions regarding relations in the given paragraph	| Generation
`task061_ropes_answer_generation`	| Answering questions regarding relations in the given paragraph	| Generation