...
for file in files[begin_task_number:end_task_number + 1]:
    if ".json" in file:
        ...
        with open(file_path, 'r') as f:
            ...
            assert type(data['Definition']) == list, f'Definition must be a list.'
            ...
            for definition in data['Definition']:
                assert all(lan in definition for lan in data['Input_language'] if
                        lan != 'English'), f'All definitions must contain non-English tasks language.'
